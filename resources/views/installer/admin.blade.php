...
@section('content')
<h2 class="mb-4 text-xl font-semibold">Create Admin Account</h2>

<form action="{{ route('install.admin.store') }}" method="post" class="space-y-4">
    @csrf
    <input type="text" name="name" placeholder="Full Name" class="p-2 w-full border" required>
    <input type="email" name="email" placeholder="Email" class="p-2 w-full border" required>
    <input type="password" name="password" placeholder="Password" class="p-2 w-full border" required>
    <input type="password" name="password_confirmation" placeholder="Confirm Password" class="p-2 w-full border" required>
    <button type="submit" class="px-4 py-2 text-white bg-green-600 rounded">Finish</button>
</form>
@endsection --}}
